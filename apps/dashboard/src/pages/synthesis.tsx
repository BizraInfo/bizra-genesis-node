...
const Synthesis: React.FC = () => {
  ...
  const [workflows, setWorkflows] = useState<SynthesisWorkflow[]>([
    {
      id: 'wf-1',
  ...
  const [selectedWorkflow, setSelectedWorkflow] = useState<SynthesisWorkflow | null>(null)
  ...
  const createNewWorkflow = () => {
    const newWorkflow: SynthesisWorkflow = {
      id: `wf-${Date.now()}`,
  ...
  const saveWorkflow = useCallback((workflow: SynthesisWorkflow) => {
    setWorkflows(prev =>
  ...
  const runSynthesis = async (workflow: SynthesisWorkflow) => {
  ...
  const getStatusColor = (status: SynthesisWorkflow['status']) => {
    switch (status) {
