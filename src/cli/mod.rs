...
use crate::sovereign_stack::SovereignModelStack;
    ...
    fn default() -> Self {
        Self {
            backend: BackendConfig {
                mode: "sovereign".to_string(), // Use sovereign model stack by default
                ollama_url: "http://localhost:11434".to_string(),
                ollama_models: vec![
                    "bizra-planner:latest".to_string(),
...
impl AgentCLI {
    ...
    pub fn new(config: CLIConfig) -> Result<Self, Box<dyn Error>> {
        ...
        let ai_backend: Arc<dyn AIBackend> = match config.backend.mode.as_str() {
            "sovereign" => {
                // Load sovereign model stack from config file
                let stack = SovereignModelStack::load_default()
                    .unwrap_or_else(|_| SovereignModelStack::default_stack());
                stack.print_summary();

                // Extract model names from sovereign stack
                ...
                tracing::info!("🧠 Sovereign mode: {} models loaded", models.len());
                ...
                let moe_config = bizra_moe::OllamaConfig {
                    ...
                    ihsan_threshold: 0.85, // Higher bar for sovereign stack
                };
...
mod tests {
    ...
    fn test_cli_config_default() {
        ...
        assert_eq!(config.backend.mode, "sovereign"); // Default is sovereign model stack
