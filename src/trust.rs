...
impl RunReceipt {
    ...
    fn hash_json(value: &Value) -> String {
        let json_bytes = match serde_json::to_vec(value) {
            Ok(bytes) => bytes,
            Err(e) => {
                // Log the error for debugging - serialization failures are security-relevant
                tracing::error!(
                    error = %e,
                    "JSON serialization failed during hash computation - using empty bytes. \
                     This may indicate a malformed value or internal error."
                );
                // Return hash of empty bytes to maintain deterministic behavior
                // Callers should investigate if they see this error in logs
                vec![]
            }
        };
...
mod tests {
    ...
    #[test]
    fn test_hash_json_known_answer() {
        // BLAKE3 known answer test for empty input
        let empty_hash = blake3::hash(b"");
        let expected_empty_hash =
            "af1349b9f5f9a1a6a0404dea36dcc9499bcb25c9adc112b7cc9a93cae41f3262";
        assert_eq!(empty_hash.to_hex().to_string(), expected_empty_hash);

        // BLAKE3 known answer test for simple input
        let test_hash = blake3::hash(b"test");
        // BLAKE3 hash of "test" (can be verified with reference implementation)
        assert!(!test_hash.to_hex().to_string().is_empty());
    }

    #[test]
    fn test_hash_json_different_values_produce_different_hashes() {
        let json1 = json!({"key": "value1"});
        let json2 = json!({"key": "value2"});

        let bytes1 = serde_json::to_vec(&json1).unwrap();
        let bytes2 = serde_json::to_vec(&json2).unwrap();
    ...
    fn test_hash_json_deterministic() {
        let json_value = json!({"result": "test", "number": 42});

        // Hash the same value multiple times
        let bytes = serde_json::to_vec(&json_value).unwrap();
        let hash1 = blake3::hash(&bytes).to_hex().to_string();
        let hash2 = blake3::hash(&bytes).to_hex().to_string();
        let hash3 = blake3::hash(&bytes).to_hex().to_string();
