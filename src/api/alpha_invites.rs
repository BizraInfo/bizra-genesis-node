...
pub async fn request_alpha_access(
    ...
    let existing = sqlx::query_scalar::<_, i64>(
        "SELECT COUNT(*) FROM alpha_requests WHERE email = $1"
    )
    .bind(&payload.email)
    .fetch_one(&pool)
    .await
    ...
    let current_count = sqlx::query_scalar::<_, i64>(
        "SELECT COUNT(*) FROM alpha_requests"
    )
    .fetch_one(&pool)
    .await
    .map_err(|e| {
...
pub async fn accept_alpha_invite(
    ...
    // Find invite
    let invite: Option<AlphaInvite> = sqlx::query_as!(
        AlphaInvite,
        r#"
    ...
    sqlx::query(
        "UPDATE alpha_invites SET status = $1, accepted_at = $2 WHERE id = $3"
    )
...
async fn send_alpha_invite_email(email: &str, name: &str, invite_code: &str) {
    ...
    println!("\nAccess the platform at: https://bizra.ai/invite/{}", invite_code);
