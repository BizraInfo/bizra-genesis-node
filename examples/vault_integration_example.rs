...
use bizra_genesis_node::secrets::{
    manager::{BackendType, SecretConfig, SecretError},
    SecretManager,
};
use std::sync::Arc;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   - Address: {}",
        prod_config.vault.as_ref().unwrap().address
    );
    println!(
        "   - Mount Path: {}",
        prod_config.vault.as_ref().unwrap().mount_path
    );
    println!(
        "   - Cache Keys: {} keys configured",
        prod_config.cache_keys.len()
    );
    ...
    println!(
        "   - Project: {}",
        kms_config.kms.as_ref().unwrap().project_id
    );
    println!(
        "   - Location: {}",
        kms_config.kms.as_ref().unwrap().location
    );
    println!(
        "   - Key Ring: {}",
        kms_config.kms.as_ref().unwrap().key_ring
    );
    println!("   - Key ID: {}", kms_config.kms.as_ref().unwrap().key_id);
    println!(
        "   - Service Account Path: {:?}",
        kms_config.kms.as_ref().unwrap().service_account_path
    );
...
impl ApplicationWithSecrets {
    ...
    pub async fn new(config: SecretConfig) -> Result<Self, SecretError> {
        let secret_manager = Arc::new(RwLock::new(SecretManager::new(config).await?));
